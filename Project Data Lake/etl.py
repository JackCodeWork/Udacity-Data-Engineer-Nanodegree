...
"""
Read data files and extract songs and artists tables from it
"""
...
"""
Read log data files and extract users, time and songplays tables from it
"""
def process_log_data(spark, input_data, output_data):
    ...
    time_table.partitionBy(["year", "month"]).parquet(output_data + "times/", mode= "overwrite")
    ...
    songplays_table = df.selectColumn("songplay_id", monotonically_increasing_id())\
                        .join(song_df, (df.song == song_df.title) & (df.artist == song_df.artist_name), how="inner")\
                        .select("songplay_id", "start_time", col("userId").alias("user_id"), "level", "song_id", 
